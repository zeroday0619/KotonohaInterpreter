...
from kotonoha.async_runtime import run as run_async
from kotonoha.config import Settings, load_settings
from kotonoha.i18n import _, available_locales, set_locale
from kotonoha.logging_setup import setup_logging


# -- shared wiring --------------------------------------------------------
def _build(settings: Settings, wave_path: Path | None = None, text_only: bool = False):
    from kotonoha.audio.capture import FileCapture, MicCapture, NullCapture
    from kotonoha.audio.denoise import build_denoiser
    from kotonoha.audio.playback import NullPlayback, Playback
    from kotonoha.audio.vad import UtteranceSegmenter, build_vad
...
def load_wave_file(path: Path, target_rate: int) -> np.ndarray:
...
def _settings(context: typer.Context) -> Settings:
...
def tui(context: typer.Context) -> None:
    from kotonoha.tui.workflow import run_unified_tui
...
def text_command(
    ...
    if not speak:
        from kotonoha.audio.playback import NullPlayback
...
def glossary_import(
    ...
    from kotonoha.store.db import GlossaryEntry, Store
...
def glossary_list(context: typer.Context) -> None:
...
def doctor(context: typer.Context) -> None:
    ...
    # discovered.
    from kotonoha.i18n import DEFAULT_LOCALE, available_locales, mo_path
    ...
    async def probe() -> None:
        from kotonoha.clients.build import build_service_group
...
def config(context: typer.Context) -> None:
...
def history_browse(context: typer.Context) -> None:
...
def history_export(
    ...
    from kotonoha.tui.history_app import export_jsonl
