...
from kotonoha.clients.base import ServiceError, ServiceTimeout
from kotonoha.clients.build import ServiceGroup, build_service_group
from kotonoha.clients.llm import GenerationStatistics
from kotonoha.clients.router import FailoverClient
from kotonoha.config import Settings
from kotonoha.core.clauses import ClauseStreamer
from kotonoha.core.events import EventBus
from kotonoha.core.lid import decide_language, decide_typed_language, route_targets
from kotonoha.core.quality import character_error_rate, is_divergent, should_cross_verify
from kotonoha.core.state import Machine, State
from kotonoha.core.zh import TraditionalChineseConverter, looks_simplified
from kotonoha.logging_setup import get_logger
from kotonoha.metrics import TurnLog, TurnMetrics
from kotonoha.prompts.asr_prompt import build_asr_context
from kotonoha.prompts.translate import build_translate_messages
from kotonoha.shmring import AudioRing
from kotonoha.store.db import Store
from kotonoha.transport import AudioPayload
...
class Orchestrator:
    ...
    async def _on_utterance(self, utterance) -> None:
    ...
    # -- one turn --------------------------------------------------------
    async def _process(self, utterance) -> None:
        metrics = TurnMetrics()
        metrics.mark("eou")
    ...
    async def submit_text(self, text: str, source_language: str | None = None) -> bool:
    ...
    async def _process_text(self, text: str, source_language: str | None) -> None:
        metrics = TurnMetrics()
    ...
    async def _watch_first_audio(self, metrics: TurnMetrics) -> None:
    ...
    # -- noise suppression -------------------------------------------------
    def _denoise(self, utterance, metrics: TurnMetrics) -> np.ndarray:
        ...
        try:
            ...
            if raw_audio.size < self.denoiser.rate // 10:
                return utterance.pcm
            from kotonoha.audio.resample import resample_once
    ...
    def _on_state_change(self, previous: State, current: State, reason: str) -> None:
