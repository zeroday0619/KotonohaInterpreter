...
from kotonoha.cli import app
from kotonoha.i18n import set_locale
...
class HealthyAsyncClient:
    ...
    async def __aenter__(self) -> HealthyAsyncClient:
        return self

    async def __aexit__(self, *exception_details) -> None:
        return None

    async def get(self, url: str) -> HealthyResponse:
        return HealthyResponse()

    async def post(self, url: str, **options) -> HealthyResponse:
        return HealthyResponse()


def test_netcheck_executes_localized_output_before_measurement_loops(monkeypatch) -> None:
