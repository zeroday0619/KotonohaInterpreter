...
from kotonoha.tui.tools_app import ToolInputError, build_tool_command
...
def test_replay_command_includes_global_and_command_options(tmp_path: Path) -> None:
...
def test_operation_command_covers_each_option(
    ...
    assert command[:5] == [sys.executable, "-m", "kotonoha.cli", "--lang", "en"]
    assert command[5:] == expected_tail


def test_glossary_import_requires_and_passes_an_existing_file(tmp_path: Path) -> None:
