...
from kotonoha.prompts.translate import SRC_MARKER, parse_llm_output


def feed(text: str, chunk: int = 3) -> tuple[list[str], ClauseStreamer]:
    cs = ClauseStreamer()
    out: list[str] = []
...
def test_long_run_without_terminator_is_cut():
    text = ("word " * 60).strip()
